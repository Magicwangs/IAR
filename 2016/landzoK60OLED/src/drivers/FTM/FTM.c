...
void FTM0_Init(double freq ,int duty)
{
    PORTC_PCR3= PORT_PCR_MUX(4); // 设置引脚C3引脚为FTM0_CH2功能
    PORTC_PCR4= PORT_PCR_MUX(4); // 设置引脚C4引脚为FTM0_CH3功能
    SIM_SCGC6|=SIM_SCGC6_FTM0_MASK;//使能FTM0时钟
    FTM0_SC=FTM_SC_CLKS(1)|FTM_SC_PS(1);//中断禁止，时钟源来自总线时钟60MHz，16分频后得到3.75MHz，CLKS=01，PS=100,CPWMS=0，递增计数
    ...
    FTM0_DEADTIME=FTM_DEADTIME_DTPS(3)|FTM_DEADTIME_DTVAL(1);  
    ...
    double temp = freq/50000;
     mod = (int)(1000*1.0/temp);
     
    FTM0_MOD=mod;//结束值，周期为（MOD-CNTIN+1）*时钟周期=10ms//     /6000000
    FTM0_C2V=0;//高电平1.5ms
...
void  FTM0_Duty_Change(int duty)
{
   FTM0_C3V=(int)(duty*1.0/1000*mod);
   FTM0_SYNC|=FTM_SYNC_SWSYNC_MASK;//使能软件触发    
}

void FTM0_Freq_Change(double freq)
{
  double temp = freq/50000;
  mod = (int)(1000*1.0/temp);
