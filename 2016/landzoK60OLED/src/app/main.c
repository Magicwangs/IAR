...
int U0_Correct=51/3.3*4095;

int pwm_freq=40000;
int pwm_gain_temp=10;
...
int Voltage_Error[2]={0,0};
int Current_Error[2]={0,0};
...
void main()
{
  ...
  *********************************************************/
   ...
   gpio_init (PORTB ,5, GPO,HIGH); //B5 初始为高电平
   
  // FTM_PWM_init(FTM0, CH2 ,2000,0); //C3
   //FTM_PWM_init(FTM0, CH3 ,2000,0); //C4
   
  

   FTM_Input_init(FTM1,CH0,Rising);  //A8输入捕捉初始化
   
   FTM0_Init(40000 ,100);  //C3、C4初始化
...
void Voltage_PID()
{
  ...
  Voltage_Error[0]=U0_Correct-U0_sample_max;
...
{  
  if(start_flag==1)
  {
    ...
    if(group_id==4)
    {
      ...
      pwm_duty=pwm_gain*test_sin[count_sin]/10;
      FTM0_Duty_Change(pwm_duty);
      count_sin++;
    }
  }
